...
const dayPrompts = [
  { q: '?Que fue lo mas dificil de hoy?', hint: 'Today the hardest thing was...' },
  ...
  { q: '?Cuales son tus planes para manana?', hint: 'Tomorrow I plan to...' },
  { q: '?Algo que te preocupo o estreso hoy?', hint: 'Today I was stressed about...' },
  { q: '?Que aprendiste hoy -- en el trabajo o en la vida?', hint: 'Today I learned that...' },
  { q: '?Con quien hablaste hoy y de que?', hint: 'Today I talked to...' },
]

const empathy = {
  primera: { title: 'Bienvenido.', body: 'Vas a aprender diferente -- no desde lecciones genericas, sino desde lo que vives tu cada dia. Tu ingles empieza hoy, donde estas.' },
  par: { title: 'Tiene todo el sentido.', body: 'Las apps populares estan disenadas para mantenerte enganchado, no para que realmente aprendas. Esta funciona al reves -- tu eres el contenido.' },
  muchas: { title: 'No es tu culpa.', body: 'Las apps no estaban disenadas para alguien como tu. Esta si. Tu ingles de hoy -- sea cual sea -- es exactamente el punto de partida correcto.' },
}

const quotes = [
  'Cada oracion que escribes en ingles es un paso real.',
  'El ingles no se aprende leyendo sobre el -- se aprende usandolo.',
  'Hoy mejor que ayer. Eso es todo lo que importa.',
  'Tu dia real es el mejor material de aprendizaje que existe.',
  'No hay errores aqui -- solo ingles en construccion.',
  'La fluidez no llega de golpe. Llega asi -- un dia a la vez.',
  'Hablar ingles no es un talento. Es un habito.',
]
...
const chatTopics = [
  ...
  { es: 'Una serie o pelicula que estas viendo', en: 'a TV show or movie you are watching' },
  ...
  { es: 'La ultima vez que usaste ingles', en: 'the last time you used English' },
]

const situations = [
  { title: 'Reunion de trabajo', desc: 'Presentas una idea a tu equipo en ingles', icon: '💼' },
  ...
  { title: 'Llamada con cliente', desc: 'Un cliente en ingles tiene un problema que resolver', icon: '📞' },
  { title: 'Hotel con problema', desc: 'Tu habitacion tiene un defecto y necesitas cambiarte', icon: '🏨' },
]
...
const duelWords = [
  ...
  { word: 'Since vs For', q: 'I have been working here ___ 3 years.', options: ['since', 'for', 'during', 'from'], correct: 1, exp: '"For" se usa con periodos de tiempo (3 years). "Since" se usa con un punto especifico en el tiempo (since 2021).' },
  ...
  { word: 'Although vs However', q: '___ it was raining, we went for a walk.', options: ['However', 'Although', 'Despite', 'Nevertheless'], correct: 1, exp: '"Although" conecta dos ideas en una oracion. "However" conecta dos oraciones separadas con punto y coma.' },
  { word: 'Much vs Many', q: 'I do not have ___ time today.', options: ['many', 'much', 'lots', 'several'], correct: 1, exp: '"Much" se usa con sustantivos incontables (time, money, water). "Many" con contables (people, ideas, meetings).' },
  ...
  { word: 'At vs In vs On', q: 'The meeting is ___ Monday at 9am.', options: ['in', 'at', 'on', 'by'], correct: 2, exp: '"On" para dias (on Monday, on Friday). "At" para horas (at 9am). "In" para meses o anos (in March, in 2024).' },
  ...
  { word: 'Passive Voice', q: 'The report ___ submitted yesterday.', options: ['was', 'is', 'were', 'had'], correct: 0, exp: 'Pasiva en pasado simple: "was/were + participio". El sujeto recibe la accion. "The report was submitted" = alguien lo envio.' },
  { word: 'Conditionals', q: 'If I ___ more time, I would learn French.', options: ['have', 'had', 'would have', 'will have'], correct: 1, exp: 'Segunda condicional: "If + past simple, would + infinitive". Expresa situaciones hipoteticas o poco probables en el presente.' },
  ...
  { word: 'So vs Such', q: 'It was ___ a great presentation.', options: ['so', 'such', 'very', 'too'], correct: 1, exp: '"Such" va antes de articulo + sustantivo (such a great idea). "So" va antes de adjetivo o adverbio sin sustantivo (so great, so quickly).' },
]
...
export default function Home() {
  ...
  async function loadFrase() {
    ...
    setView('frase')
    try {
      const resp = await callAI(`Eres coach de ingles. Genera UNA expresion o phrasal verb util para nivel B1 relacionado con trabajo o vida cotidiana. Responde en este formato EXACTO, una linea por seccion:\n\nFRASE: [la expresion en ingles]\nSIGNIFICADO: [que significa en espanol, 1 linea]\nCUANDO USARLA: [1 situacion concreta]\nEJEMPLO: [oracion de ejemplo en ingles]\n\nElige algo practico, no obvio.`)
      setFrase(resp)
  ...
  async function submitObText() {
    ...
    setObLoading(true)
    try {
      const resp = await callAI(`Eres un coach de ingles cercano y motivador. RESPONDE EN ESPANOL natural y calido.\n\nPrimero verifica si el texto esta en ingles (aunque sea imperfecto). Si no es ingles, responde exactamente: INVALID\n\nSi es ingles valido, da:\n1. Una observacion positiva MUY especifica sobre algo concreto del texto\n2. Una sola mejora usando sus propias palabras\n\nTono: amigo bilingue, calido, directo. Sin tecnicismos. Maximo 3 oraciones.\n\nTexto: "${obText}"`)
      if (resp.trim().startsWith('INVALID')) {
        alert('Escribe algo en ingles -- aunque sea una sola oracion. No importa si tiene errores.')
  ...
  async function submitDay() {
    ...
    try {
      ...
      if (resp.trim().startsWith('TOO_SHORT')) {
        setDayWarning('Cuentame un poco mas -- con dos oraciones sobre tu dia es suficiente.')
        setDayLoading(false); return
      }
  ...
  async function genReto(text) {
    try {
      const resp = await callAI(`Eres coach de ingles. RESPONDE EN ESPANOL natural. Texto del aprendiz: "${text}"\n\nIdentifica el patron mas util:\n**El patron:** [que es y cuando usarlo -- 1-2 oraciones simples]\n**Asi se veria mejor:** [toma algo de su texto y muestralo mejorado]\n**Tu turno:** [una situacion simple para practicar ese patron]\n\nMaximo 80 palabras. Tono: amigo que te corrige con carino.`)
      setRetoPattern(resp)
    } catch (e) { }
  }
  ...
  async function submitVocab() {
    ...
    try {
      const resp = await callAI(`Eres coach de ingles. RESPONDE EN ESPANOL natural.\n\nEl aprendiz debia usar una de estas palabras: ${sessionVocab.map(w => w.word).join(', ')}.\nEscribio: "${vocabPractice}"\n\n?Uso alguna de esas palabras? ?Esta en ingles? Si no, responde: INVALID\n\nSi esta bien: feedback en 2-3 oraciones. ?La uso correctamente? ?Que mejorarias? Se especifico y motivador.`)
      if (resp.trim().startsWith('INVALID')) {
        setVocabWarning(`Usa una de estas palabras en ingles: ${sessionVocab.map(w => w.word).join(', ')}.`)
  ...
  async function submitReto() {
    ...
    try {
      const resp = await callAI(`Eres coach de ingles. RESPONDE EN ESPANOL natural.\n\nPatron a practicar: "${retoPattern}"\nEl aprendiz escribio: "${retoInput}"\n\n?Intento practicar el patron en ingles? Si no, responde: INVALID\n\nSi esta bien: correccion y animo en 2-3 oraciones. Muestra version mejorada si necesario.`)
      if (resp.trim().startsWith('INVALID')) {
        setRetoWarning('Intenta practicar el patron en ingles -- aunque sea una oracion corta.')
        setRetoLoading(false); return
      }
      setRetoResp(resp); addLearned('Gramatica: patron del dia practicado')
    } catch (e) { }
  ...
  async function submitCierre() {
    ...
    try {
      const resp = await callAI(`Eres coach de ingles. RESPONDE EN ESPANOL cercano y personal.\n\nEl aprendiz ${state?.name || ''}completo su sesion. Escribio sobre: "${sessionDayText.substring(0, 150)}". Vocabulario trabajado: ${sessionVocab.map(w => w.word).join(', ') || 'general'}.\n\nEscribe 2-3 oraciones MUY personales:\n1. Menciona algo especifico de lo que escribio hoy\n2. Un consejo concreto para manana\n\nNada generico. Habla como alguien que realmente leyo su texto.`)
  ...
  async function endChat() {
    ...
    try {
      ...
      const resp = await callAI(`Eres coach de ingles. RESPONDE EN ESPANOL natural y calido.\n\nEl aprendiz tuvo una conversacion en ingles sobre "${topicLabel}". Sus mensajes: "${history}"\n\nFeedback personal en 3-4 oraciones:\n1. Algo especifico que hizo bien (menciona sus palabras reales)\n2. Una expresion o patron para mejorar con ejemplo\n3. Cierre motivador\n\nTono: amigo que estuvo escuchando toda la conversacion.`)
  ...
  async function endSit() {
    ...
    try {
      ...
      const resp = await callAI(`Eres coach de ingles. RESPONDE EN ESPANOL cercano.\n\nEl aprendiz hizo un roleplay de "${situacion?.title}". Sus intervenciones: "${history}"\n\nFeedback en 3-4 oraciones:\n1. Lo que manejo bien en la situacion (especifico)\n2. Una frase o expresion clave que le hubiera servido\n3. Cierre motivador\n\nTono: amigo que vio toda la actuacion.`)
  ...
  async function submitFrase() {
    ...
    try {
      ...
      const resp = await callAI(`Eres coach de ingles. RESPONDE EN ESPANOL natural.\n\nLa frase del dia era: "${phraseWord}"\nEl aprendiz la uso en: "${fraseInput}"\n\n?La uso correctamente? ?Suena natural? Feedback en 2-3 oraciones. Tono amigable y motivador.`)
      setFraseResp(resp); addLearned(`Frase del dia: ${phraseWord}`)
    } catch (e) { }
  }

  const reviewCards = (state?.vocab || []).slice(-9).slice(0, 3)
  const greeting = (() => { const h = new Date().getHours(); return h < 12 ? 'Buenos dias' : h < 20 ? 'Buenas tardes' : 'Buenas noches' })()
        ...
        <div className="fade-up" style={{ maxWidth: 520, width: '100%', margin: '0 auto', padding: '2rem 1.25rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          {obStep === 1 && (
            <div key="ob1" className="fade-up">
              <h1 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 'clamp(1.7rem,5vw,2.2rem)', lineHeight: 1.2, marginBottom: '0.75rem', letterSpacing: '-0.03em', fontWeight: 400 }}>?Cuantas veces has intentado aprender ingles?</h1>
          ...
          {obStep === 3 && (
            <div key="ob3" className="fade-up">
              <h2 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 'clamp(1.5rem,4vw,1.9rem)', lineHeight: 1.25, marginBottom: '0.75rem', letterSpacing: '-0.03em', fontWeight: 400 }}>?Como te llamas?</h2>
              <p style={{ fontSize: '0.9rem', color: 'var(--muted)', marginBottom: '1.5rem', lineHeight: 1.65 }}>Para que tu coach te conozca desde el primer dia.</p>
          ...
          {obStep === 4 && (
            <div key="ob4" className="fade-up">
              <ModuleTag label="TU PRIMER DIA EN INGLES" />
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ maxWidth: 560, width: '100%', margin: '0 auto', padding: '1.5rem 1.25rem', flex: 1, display: 'flex', flexDirection: 'column', gap: 14, overflowY: 'auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            {[
              { icon: '🔥', val: state.streak || 0, label: 'dias de racha', color: 'var(--amber)', bg: 'var(--amber-l)' },
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                ...
                { icon: '💬', title: 'Chat libre', desc: 'Conversa en ingles sobre lo que quieras', time: '5-10 min', color: 'var(--blue)', onClick: () => setView('chat-select') },
                { icon: '⚡', title: 'Duelo rapido', desc: '5 preguntas de gramatica real', time: '2 min', color: 'var(--amber)', onClick: startDuel },
                { icon: '🎭', title: 'Situacion real', desc: 'Roleplay de trabajo, viaje o vida real', time: '5-8 min', color: 'var(--purple)', onClick: () => setView('sit-select') },
                { icon: '🎯', title: 'Frase del dia', desc: 'Una expresion nueva y util', time: '2 min', color: 'var(--g)', onClick: loadFrase },
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--r)', border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            <div style={{ padding: '0.9rem 1.2rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '0.9rem', fontWeight: 500, color: 'var(--text)' }}>Ultimos aprendizajes</span>
            ...
            {(!state.learned || state.learned.length === 0)
              ? <div style={{ padding: '1.5rem 1.2rem', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '0.88rem', color: 'var(--muted)', lineHeight: 1.6 }}>Aqui aparecera lo que vayas aprendiendo.<br />Completa tu primera sesion.</div>
              </div>
  ...
  if (view === 'session') {
    ...
    const stepTitles = { day: 'Mi dia en ingles', vocab: 'Vocabulario vivo', reto: 'Mini reto', review: 'Revision rapida', cierre: 'Cierre del dia' }
    ...
    return (
      <>
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
          ...
          {sessionStep === 'day' && (
            <PageContent>
              <ModuleTag label="MODULO 1 * MI DIA EN INGLES" />
              ...
              <p style={{ fontSize: '0.88rem', color: 'var(--muted)', marginBottom: '1.5rem', lineHeight: 1.65 }}>En ingles, como puedas. La IA adapta el resto de la sesion a lo que escribas.</p>
          ...
          {sessionStep === 'vocab' && (
            <PageContent>
              <ModuleTag label="MODULO 2 * VOCABULARIO VIVO" />
              ...
              <p style={{ fontSize: '0.88rem', color: 'var(--muted)', marginBottom: '1.4rem', lineHeight: 1.65 }}>No de un diccionario generico -- de lo que tu mismo escribiste.</p>
              {vocabWords.length > 0 && (
          ...
          {sessionStep === 'reto' && (
            <PageContent>
              <ModuleTag label="MODULO 3 * MINI RETO" color="var(--amber)" bg="var(--amber-l)" />
              <h2 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 'clamp(1.4rem,4vw,1.85rem)', lineHeight: 1.25, marginBottom: '0.75rem', letterSpacing: '-0.03em', fontWeight: 400 }}>Tu patron del dia</h2>
              <p style={{ fontSize: '0.88rem', color: 'var(--muted)', marginBottom: '1.4rem', lineHeight: 1.65 }}>Un solo patron, sacado de tu texto. Asi mejora el ingles real.</p>
              {!retoPattern
                  ...
                  {!retoResp ? (
                    <>
                      {(() => {
                        ...
                        const pattern = lines.find(l => l.includes('patron') || l.includes('Patron'))?.replace(/\*\*/g, '') || ''
                        const challenge = lines.find(l => l.includes('turno') || l.includes('intentalo'))?.replace(/\*\*/g, '') || ''
                        return (
                          <>
                            {pattern && (
                              <div style={{ background: 'var(--surface)', border: '1px solid #F0D9B5', borderRadius: 'var(--r)', padding: '1rem 1.2rem', marginBottom: 16, boxShadow: 'var(--shadow-sm)' }}>
                                <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--amber)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>El patron</div>
          ...
          {sessionStep === 'review' && (
            <PageContent>
              <ModuleTag label="MODULO 4 * REVISION RAPIDA" color="var(--coral)" bg="var(--coral-l)" />
                  ...
                  return (
                    <>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
                        <h2 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 'clamp(1.3rem,4vw,1.7rem)', letterSpacing: '-0.03em', fontWeight: 400 }}>Revision rapida</h2>
                      ...
                      <div onClick={() => !reviewFlipped && setReviewFlipped(true)}
                        ...
                        {reviewFlipped
                          ...
                          : <div style={{ fontSize: '0.82rem', color: 'var(--muted2)', display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span>👆</span> Toca para ver la definicion
                          </div>
                        }
                      </div>
                      {reviewFlipped
                        ? <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                          {[['✓  Lo tenia', 'var(--g)', 'var(--g-light)'], ['↺  Repasar', 'var(--coral)', 'var(--coral-l)']].map(([label, color, bg]) => (
          ...
          {sessionStep === 'cierre' && (
            <PageContent>
              <ModuleTag label="MODULO 5 * CIERRE DEL DIA" />
          ...
          {sessionStep === 'done' && (
            <div className="fade-up" style={{ maxWidth: 560, width: '100%', margin: '0 auto', padding: '2rem 1.25rem', flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
              ...
              <div style={{ textAlign: 'center' }}>
                <h2 style={{ fontFamily: "'Instrument Serif',serif", fontSize: 'clamp(1.6rem,5vw,2rem)', fontWeight: 400, letterSpacing: '-0.03em', marginBottom: 8 }}>Sesion completada{firstName ? `, ${firstName}` : ''}.</h2>
                <p style={{ fontSize: '0.9rem', color: 'var(--muted)', lineHeight: 1.6 }}>
                  Racha: <strong style={{ color: 'var(--g)', fontWeight: 600 }}>{state.streak || 1} {(state.streak || 1) === 1 ? 'dia' : 'dias'}</strong>
                </p>
              </div>
              <div style={{ width: '100%', background: 'var(--surface)', borderRadius: 'var(--r)', border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
                ...
                {[['Mi dia en ingles', 'Texto libre analizado', 'var(--purple)'], ['Vocabulario vivo', `${vocabWords.length} palabras aprendidas`, 'var(--g)'], ['Mini reto', 'Patron gramatical practicado', 'var(--amber)'], ['Revision rapida', `${reviewCards.length} tarjetas repasadas`, 'var(--coral)'], ['Cierre del dia', 'Resumen personalizado', 'var(--g)']].map(([title, desc, color], i, arr) => (
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="fade-up" style={{ maxWidth: 560, width: '100%', margin: '0 auto', padding: '1.8rem 1.25rem', flex: 1 }}>
          <p style={{ fontSize: '0.9rem', color: 'var(--muted)', marginBottom: '1.5rem', lineHeight: 1.65 }}>Elige un tema y conversamos en ingles. Te corrijo al final -- mientras, solo habla.</p>
  ...
  if (view === 'sit-select') return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
        <header style={{ background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--border)', padding: '0 1.25rem', display: 'flex', alignItems: 'center', gap: 12, height: 54 }}>
          <button onClick={() => setView('home')} className="pressable" style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: 18, padding: '6px 8px', borderRadius: 'var(--r-sm)', lineHeight: 1 }}>←</button>
          <span style={{ fontFamily: "'Instrument Serif',serif", fontSize: '1.1rem', letterSpacing: '-0.02em' }}>Situacion real</span>
        </header>
        <div className="fade-up" style={{ maxWidth: 560, width: '100%', margin: '0 auto', padding: '1.8rem 1.25rem', flex: 1 }}>
          <p style={{ fontSize: '0.9rem', color: 'var(--muted)', marginBottom: '1.5rem', lineHeight: 1.65 }}>Elige una situacion y la practicamos -- yo sere el otro personaje.</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {situations.map(sit => (
  ...
  if (view === 'duelo') {
    ...
    return (
      <>
        <style>{CSS}</style>
        <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
          <TopBar step={duelIdx + 1} total={5} onBack={() => setView('home')} title="Duelo rapido" />
  ...
  if (view === 'frase') {
    ...
    const when = getField('CUANDO USARLA:')
    ...
    return (
      <>
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
          <header style={{ background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--border)', padding: '0 1.25rem', display: 'flex', alignItems: 'center', gap: 12, height: 54 }}>
            <button onClick={() => setView('home')} className="pressable" style={{ background: 'none', border: 'none', color: 'var(--muted)', fontSize: 18, padding: '6px 8px', borderRadius: 'var(--r-sm)', lineHeight: 1 }}>←</button>
            <span style={{ fontFamily: "'Instrument Serif',serif", fontSize: '1.1rem', letterSpacing: '-0.02em' }}>🎯 Frase del dia</span>
          </header>
          <div className="fade-up" style={{ maxWidth: 560, width: '100%', margin: '0 auto', padding: '1.8rem 1.25rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
            {fraseLoading || !frase
              ? <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 14, color: 'var(--muted)' }}>
                <Dots /><span style={{ fontSize: '0.9rem' }}>Buscando una expresion util para ti...</span>
              </div>
              : frase === 'ERROR'
                ...
                : <>
                  <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: '1.4rem', marginBottom: 20, boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column', gap: 14 }}>
                    ...
                    {when && (
                      <div>
                        <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Cuando usarla</div>
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div className="fade-up" style={{ maxWidth: 560, width: '100%', margin: '0 auto', padding: '1.5rem 1.25rem', flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Stats */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            {[['🔥', state.streak || 0, 'Racha', 'dias'], ['📚', state.vocab?.length || 0, 'Vocabulario', 'palabras'], ['✓', state.sessions?.length || 0, 'Sesiones', 'completadas']].map(([icon, val, label, sub]) => (
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--r)', border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
            ...
            {(!state.learned || state.learned.length === 0)
              ? <div style={{ padding: '2.5rem', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '0.88rem', color: 'var(--muted)', lineHeight: 1.65 }}>Aun no hay registros.<br />Completa tu primera sesion.</div>
              </div>
